...
Function Export-AzSentinelRulesNeedingUpdates ($workspaceName, $resourceGroupName, $fileName) {
    ...
    [void]$outputObject.Columns.Add('Update', [string]::empty.GetType() )
    [void]$outputObject.Columns.Add('Name', [string]::empty.GetType() )
    [void]$outputObject.Columns.Add('Type', [string]::empty.GetType() )
    ...
    [void]$outputObject.Columns.Add('AlertTacticsColumnNameChanged', [string]::empty.GetType() )
    [void]$outputObject.Columns.Add('AlertSeverityColumnNameChanged', [string]::empty.GetType() )
    [void]$outputObject.Columns.Add('SuppressionDurationChanged', [string]::empty.GetType() )
    [void]$outputObject.Columns.Add('SuppressionEnabledChanged', [string]::empty.GetType() )
    [void]$outputObject.Columns.Add('EventGroupingSettingsChanged', [string]::empty.GetType() )
        ...
        if ($null -ne $rule.properties.templateVersion) {
            ...
            if ($null -eq $foundTemplate) {
                ...
                if ($null -ne $foundTemplate) {
                    ...
                    $type = $foundTemplate.properties.template.resources.kind
            ...
            if (($null -ne $template) -and (-not [string]::IsNullOrWhiteSpace($templateVersion))) {
                ...
                if (($null -ne $templateVersion) -and ($ruleVersion -ne $templateVersion)) {
                    ...
                    $newRow.SeverityChanged = (($rule.properties.severity -ne $template.severity) -and ($null -ne $template.severity ))
                    $newRow.QueryChanged = (($rule.properties.query -ne $template.query) -and ($null -ne $template.query ))
                    $newRow.QueryFrequencyChanged = (($rule.properties.queryFrequency -ne $template.queryFrequency) -and ($null -ne $template.queryFrequency ))
                    $newRow.QueryPeriodChanged = (($rule.properties.queryPeriod -ne $template.queryPeriod) -and ($null -ne $template.queryPeriod ))
                    $newRow.TriggerOperatorChanged = (($rule.properties.triggerOperator -ne $template.triggerOperator) -and ($null -ne $template.triggerOperator ))
                    $newRow.TriggerThresholdChanged = (($rule.properties.triggerThreshold -ne $template.triggerThreshold) -and ($null -ne $template.triggerThreshold ))
                    $newRow.AlertDetailNameChanged = ($rule.properties.alertDetailsOverride.alertDisplayNameFormat -ne $template.alertDetailsOverride.alertDisplayNameFormat)
                    ...
                    $newRow.AlertTacticsColumnNameChanged = (($rule.properties.alertDetailsOverride.alertTacticsColumnName -ne $template.alertDetailsOverride.alertTacticsColumnName) -and ($null -ne $template.alertDetailsOverride.alertTacticsColumnName ))
                    $newRow.AlertSeverityColumnNameChanged = (($rule.properties.alertDetailsOverride.alertSeverityColumnName -ne $template.alertDetailsOverride.alertSeverityColumnName) -and ($null -ne $template.alertDetailsOverride.alertTacticsColumnName ))
                    $newRow.suppressionDurationChanged = (($rule.properties.suppressionDuration -ne $template.suppressionDuration) -and ($null -ne $template.suppressionDuration ))
                    $newRow.suppressionEnabledChanged = ($rule.properties.suppressionEnabled -ne $template.suppressionEnabled -and ($null -ne $template.suppressionEnabled ))
                    $newRow.eventGroupingSettingsChanged = ($rule.properties.eventGroupingSettings -ne $template.eventGroupingSettings -and ($null -ne $template.eventGroupingSettings )) 
